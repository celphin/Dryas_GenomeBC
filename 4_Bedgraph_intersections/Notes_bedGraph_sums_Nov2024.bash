...
cp -r /home/celphin/projects/rpp-rieseber/celphin/Dryas/Methylation_calling/May2021_Parents/bismark_methylation_calls/bedGraph* ~/scratch/Dryas/
...
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > W_ALAS.bedGraph

bedtools unionbedg -i C*ALAS*.deduplicated.bedGraph > C_ALAS_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > C_ALAS.bedGraph

#---------------
bedtools unionbedg -i W*LA*.deduplicated.bedGraph > W_Swed_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > W_Swed.bedGraph

bedtools unionbedg -i C*LA*.deduplicated.bedGraph > C_Swed_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > C_Swed.bedGraph

#---------------
bedtools unionbedg -i W*CASS*.deduplicated.bedGraph > W_CASS_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > W_CASS.bedGraph

bedtools unionbedg -i C*CASS*.deduplicated.bedGraph  > C_CASS_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > C_CASS.bedGraph

#---------------
bedtools unionbedg -i W*WILL*.deduplicated.bedGraph > W_WILL_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > W_WILL.bedGraph

bedtools unionbedg -i C*WILL*.deduplicated.bedGraph > C_WILL_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > C_WILL.bedGraph

#---------------
bedtools unionbedg -i W*FERT*.deduplicated.bedGraph > W_FERT_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > W_FERT.bedGraph

bedtools unionbedg -i C*FERT*.deduplicated.bedGraph > C_FERT_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > C_FERT.bedGraph

#---------------
bedtools unionbedg -i W*DRY*.deduplicated.bedGraph > W_DRY_total.bedGraph

awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > W_DRY.bedGraph

bedtools unionbedg -i C*DRY*.deduplicated.bedGraph > C_DRY_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > C_DRY.bedGraph

#---------------
bedtools unionbedg -i W*MEAD*.deduplicated.bedGraph > W_MEAD_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > W_MEAD.bedGraph

bedtools unionbedg -i C*MEAD*.deduplicated.bedGraph > C_MEAD_total.bedGraph
awk 'OFS="\t" {sum=0; for (col=4; col<=NF; col++) sum += $col; print $1, $2, $3, sum/(NF-4+1); }' > C_MEAD.bedGraph
