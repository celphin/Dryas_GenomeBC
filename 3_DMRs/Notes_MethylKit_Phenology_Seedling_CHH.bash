...
myobj <- methRead(
  ...
  sample.id = as.list(SE_d.f$sample_ids), 
  assembly = "Dryas",
  treatment = as.numeric(SE_d.f$treatment),
  dbdir = getwd(),
  context = "CHH", 
  dbtype = "tabix",
  resolution = "base", 
...
#print(myobj)
save(myobj, file = "SE_MethylKit_data_CHH.RData")

#--------------------------------------------------
# DMRs for Seedlings

load("SE_MethylKit_data_CHH.RData")

SE_myobj <-  myobj

meth=unite(SE_myobj, destrand=FALSE)
save(meth, file = "SE_united_CHH.RData")

#---------------
# plotting
load("SE_united_CHH.RData")
pdf("SE_PCA_PC1_PC2_CHH.pdf", width = 10, height = 8)
PCASamples(meth)
dev.off()
pdf("SE_Dendrogram_plot_CHH.pdf", width = 10, height = 8)
...
library(vegan)
pdf("SE_PCA_PC1_PC2_ordiplot_CHH.pdf", width = 10, height = 8)
...
head(lowtiles[[1]],3)
save(lowtiles, file = "SE_MethylKit_tiles_lowcov_CHH.RData")
load("SE_MethylKit_tiles_lowcov_CHH.RData")

#------------------------
methtiles=unite(lowtiles, destrand = FALSE, min.per.group = NULL, chunk.size = 3e+05)
save(methtiles, file = "SE_united_lowtiles_CHH.RData")
load("SE_united_lowtiles_CHH.RData")

#---------------------------
# calculate DMRs

myDifftiles=calculateDiffMeth(methtiles, overdispersion="MN", test="Chisq")
save(myDifftiles, file = "SE_MethDiffRegions_CHH.RData")
load("SE_MethDiffRegions_CHH.RData")
...
# CHH with over dispersion and covariates?
# [1] 2
# [1] 1
# [1] 1

# overdip and chisq Sweden only
# [1] 39
# [1] 23
# [1] 16
...
write.table(SE_W_C_DMRs, "Methylkit_SE_W_C_10_DMRs_CHH.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(SE_W_C_DMRs, "Methylkit_SE_W_C_25_DMRs_CHH.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# Print the object to check the result
#print(myobj)
save(myobj, file = "Pheno_MethylKit_CHH_data.RData")

load("Pheno_MethylKit_CHH_data.RData")
...
save(meth, file = "Pheno_united_CHH.RData")

#----------
# plotting
load("Pheno_united_CHH.RData")
pdf("Pheno_CHH_PCA_PC1_PC2.pdf", width = 10, height = 8)
PCASamples(meth)
dev.off()
pdf("Pheno_CHH_Dendrogram_plot.pdf", width = 10, height = 8)
clusterSamples(meth, dist="correlation", method="ward.D2", plot=TRUE)
dev.off()
...
library(vegan)
pdf("Pheno_CHH_PCA_PC1_PC2_ordiplot.pdf", width = 10, height = 8)
fig.allDataPCA <- ordiplot(allDataPCA, choices = c(1, 2), type = "none", display = "sites", cex = 0.5, xlim = c(-400, 200), xlab = "", ylab = "", xaxt = "n", yaxt = "n") #Use ordiplot to create base biplot. Do not add any points
points(fig.allDataPCA, "sites", col = c(rep("blue", times = 10), rep("red", times = 10)), pch = c(rep(16, times = 10), rep(17, times = 10)), cex = 3) #Add each sample. Darker samples are ambient, lighter samples are elevated pCO2
...
head(lowtiles[[1]],3)
save(lowtiles, file = "Pheno_MethylKit_tiles_lowcov_CHH.RData")
load("Pheno_MethylKit_tiles_lowcov_CHH.RData")

#------------------------
methtiles=unite(lowtiles, destrand=FALSE)
save(methtiles, file = "Pheno_united_lowtiles_CHH.RData")
load("Pheno_united_lowtiles_CHH.RData")

#---------------------------
# calculate DMRs

myDifftiles=calculateDiffMeth(methtiles, overdispersion="MN", covariates=d.f$covariates, test="Chisq")
save(myDifftiles, file = "Pheno_MethDiffRegions_CHH.RData")
load("Pheno_MethDiffRegions_CHH.RData")
...
# with overdispersion and covariates 
# [1] 14683
# [1] 27
# [1] 14656

# with overdispersion and chisq 
# [1] 15563
# [1] 30
# [1] 15533
...
write.table(Pheno_DMRs, "Methylkit_Pheno_10_DMRs_CHH.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# with overdispersion and chisq - says CHH?
# total 3962 - all hyper

[1] 3962

Pheno_DMRs <- getData(myDifftiles25p)
write.table(Pheno_DMRs, "Methylkit_Pheno_25_DMRs_CHH.txt", sep = "\t", quote = FALSE, row.names = FALSE)
